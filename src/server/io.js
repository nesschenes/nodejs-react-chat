...
export const socketio = async (io, axios, config1) => {
      ...
      Redisclient.get("chatRoomUsersList", (err, reply) => {
        ...
        Redisclient.set(
          ...
          (err, reply) => {
            ...
            socket.broadcast
              .to("chatPage")
              .emit("chatRoomUsers", { user: payload, whichone: dec });
            //console.log(payload)
          }
        );
      });
          ...
          Redisclient.get("chatRoomUsersList", (err, reply) => {
            ...
            Redisclient.set(
              ...
              (err, reply) => {
                ...
                socket.broadcast
                  .to("chatPage")
                  .emit("chatRoomUsers", { user: payload, whichone: name });
                //console.log(payload)
              }
            );
          });
    ...
    socket.on("chatPage", (res) => {
      ...
      socket.join("chatPage", () => {
        ...
        Redisclient.get("chatRoomUsersList", (err, reply) => {
          ...
          const payload = JSON.parse(reply);
          const name = res.account;
          const new1 = Object.assign(payload, {
            [name]: { avatar: res.avatar, name: res.name },
          });
          Redisclient.set(
            "chatRoomUsersList",
            JSON.stringify(new1),
            (err, reply) => {
              socket.broadcast
                .to("chatPage")
                .emit("userEnter", { user: res.name }); //顯示哪個使用者進入
              socket.emit("chatRoomUsers", { user: new1, whichone: name });
              socket.broadcast
                .to("chatPage")
                .emit("chatRoomUsers", { user: new1, whichone: name }); //更新所有在聊天室的使用者列表
              //console.log(new1)
            }
          );
        });
      ...
      Redisclient.get("chatRoomUsersList", (err, reply) => {
        ...
        Redisclient.set(
          ...
          (err, reply) => {
            socket.broadcast
              .to("chatPage")
              .emit("userLeave", { user: payload.name });
            socket.emit("chatRoomUsers", { user: payload, whichone: name });
            socket.broadcast
              .to("chatPage")
              .emit("chatRoomUsers", { user: payload, whichone: name });
